...
class EpisodeWriter:
    ...
    def _write_episode(self, meta: EpisodeMeta, steps: Iterable[Mapping[str, Any]]) -> Path:
        ...
        meta_array: NDArray[np.object_] = np.empty((), dtype=object)
        meta_array[()] = asdict(meta)
        steps_array: NDArray[np.object_] = np.array(steps_list, dtype=object)
        path = self._episode_path(meta)
        if self._compress:
            np.savez_compressed(path, meta=meta_array, steps=steps_array)
        else:
            np.savez(path, meta=meta_array, steps=steps_array)
        return path
